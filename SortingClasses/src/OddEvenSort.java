...
public class OddEvenSort<T extends Comparable<T>> implements Sorts {
	/**
	 * Member Variables, keeping Track of Data for my GUI.
	 */
	private final String NAME = "Odd-Even Sort";
	...
	public OddEvenSort(ArrayList<T> arrayIn) {
		...
		sort();
		System.out.println("OddEVen" + numElements);
	}
	...
	private ArrayList<T> sortHelper(ArrayList<T> arrayListInput) {
		for (int i = 0; i < (arrayListInput.size() / 2); i++) {
			// evens including 0
			for (int j = 0; (j + 1) < arrayListInput.size(); j += 2) {
				// compare
				this.numComparisons++;
			...
			// odd only
			for (int j = 1; (j + 1) < arrayListInput.size(); j += 2) {
				// compare
				this.numComparisons++;
				if (arrayListInput.get(j).compareTo(arrayListInput.get(j + 1)) > 0) {
					// swap
